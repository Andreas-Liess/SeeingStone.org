...
function signature(buf, atF, amp = 0.5) {
  addSine(buf, atF, 90, 396, 0.34 * amp, 0.62, 0.002);
  addSine(buf, atF, 90, 594, 0.20 * amp, 0.46, 0.002);
  addSine(buf, atF + 6, 90, 792, 0.09 * amp, 0.30, 0.002);
  addSine(buf, atF, 60, 66, 0.40 * amp, 0.24, 0.001);   /* body */
}
...
const FILMS = {
  ...
  '40-minutes': (buf, rng) => {
    addRoomTone(buf, 0, 435, 0.10, rng);
    ...
    addRoomTone(buf, 545, 900, 0.07, rng);
  ...
  anniversary: (buf, rng) => {
    addRoomTone(buf, 0, 660, 0.10, rng);
    ...
    addRoomTone(buf, 700, 1150, 0.07, rng);
...
function writeWav(path, samples) {
  ...
  /* Leave headroom rather than normalising to full scale — this is ambience. */
  const gain = peak > 0 ? Math.min(1, 0.72 / peak) : 1;
